...
class A2C():
    def __init__(self, args):
        ...
        self.value = Value(self.d_state)
        logging.info('Value: {}'.format(self.value))

        self.N = args.N

        self.actor_learning_rate = args.actor_learning_rate
        self.critic_learning_rate = args.critic_learning_rate
        self.actor = eval('optim.' + args.optimizer)(self.policy.parameters(), lr=self.actor_learning_rate)
        self.critic = eval('optim.' + args.optimizer)(self.value.parameters(), lr=self.critic_learning_rate)
    ...
    def plot_rewards(self):
        ...
        plt.xlabel('Episode')
        plt.title('{}, N={}'.format(self.env.spec.id, self.N))
    ...
    def train(self):
        ...
        for i in range(1, self.train_episodes + 1):
            ...
            rewards, log_probs, states = self.generate_episode()
            reward_batch = FloatTensor(rewards)
            log_prob_batch = torch.cat(log_probs)
            state_batch = FloatTensor(np.concatenate(states))
            T = reward_batch.size()[0]

            value_batch = self.value(Variable(state_batch))
            if self.N < T:
                R = (self.discount ** self.N) * torch.cat([value_batch.data[self.N:, 0], torch.zeros(self.N)])
            else:
                R = torch.zeros(T)
            ...
            for t in range(T):
                M = min(t + self.N, T)
                partial_rewards[t] = torch.sum((self.discount ** torch.arange(M - t)) * reward_batch[t:M])

            R += self.reward_scale * partial_rewards

            actor_loss = -torch.mean((Variable(R) - value_batch.detach()) * log_prob_batch)
            self.actor.zero_grad()
            actor_loss.backward()
            self.actor.step()

            critic_loss = nn.MSELoss()(value_batch, Variable(R))
            self.critic.zero_grad()
            critic_loss.backward()
            self.critic.step()

            total_reward += sum(rewards)
            total_loss += actor_loss.data[0]
